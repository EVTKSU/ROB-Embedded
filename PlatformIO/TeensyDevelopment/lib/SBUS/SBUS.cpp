...
SBUS::SBUS(HardwareSerial& bus) { _bus = &bus; }
...
// begin the serial port for SBUS
    ...
    defined(__MK20DX256__)  // Teensy 3.0 || Teensy 3.1/3.2
  _bus->begin(_sbusBaud, SERIAL_8E1_RXINV_TXINV);
  SERIALPORT = _bus;
#elif defined(__IMXRT1062__) || defined(__IMXRT1052__) || \
    defined(__MK64FX512__) || defined(__MK66FX1M0__) ||   \
    defined(__MKL26Z64__)  // Teensy 4.0 || Teensy 4.0 Beta || Teensy 3.5 ||
                           // Teensy 3.6 || Teensy LC
  _bus->begin(_sbusBaud, SERIAL_8E2_RXINV_TXINV);
#elif defined(STM32L496xx) || defined(STM32L476xx) || defined(STM32L433xx) || \
    defined(STM32L432xx)             // STM32L4
  ...
  _bus->begin(_sbusBaud, SERIAL_8E2);
    ...
    defined(__AVR_ATmega32U4__)   // Arduino Mega 2560, 328P or 32u4
  _bus->begin(_sbusBaud, SERIAL_8E2);
...
bool SBUS::readCal(float* calChannels, bool* failsafe, bool* lostFrame) {
  ...
  if (read(&channels[0], failsafe, lostFrame)) {
    ...
    for (uint8_t i = 0; i < _numChannels; i++) {
      ...
      // if (_useReadCoeff[i]) {
      //	calChannels[i] =
      //PolyVal(_readLen[i],_readCoeff[i],calChannels[i]);
      // }
    }
    // return true on receiving a full packet
    return true;
...
bool SBUS::parse() {
  ...
  while (_bus->available() > 0) {
    ...
    // find the header
    if (_parserState == 0) {
      if ((_curByte == _sbusHeader) &&
          ((_prevByte == _sbusFooter) ||
           ((_prevByte & _sbus2Mask) == _sbus2Footer))) {
        _parserState++;
      } else {
        _parserState = 0;
      }
      ...
      if ((_parserState - 1) == _payloadSize) {
        if ((_curByte == _sbusFooter) ||
            ((_curByte & _sbus2Mask) == _sbus2Footer)) {
          _parserState = 0;
          return true;
        } else {
          _parserState = 0;
...
void SBUS::scaleBias(uint8_t channel) {
  _sbusScale[channel] =
      2.0f / ((float)_sbusMax[channel] - (float)_sbusMin[channel]);
  _sbusBias[channel] =
      -1.0f *
      ((float)_sbusMin[channel] +
       ((float)_sbusMax[channel] - (float)_sbusMin[channel]) / 2.0f) *
      _sbusScale[channel];
}
